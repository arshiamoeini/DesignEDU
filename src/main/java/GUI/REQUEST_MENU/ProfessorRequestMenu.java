...
public class ProfessorRequestMenu implements PanelDesigner {
        ...
        recommendationField.add(new DemoList() {
            {
                ...
                designTopics();
                for (Student student: Command.getInstance().getRecommendedStudent()) {
                    addRow(student);
                }
            }
    ...
    private void educationalRequestsInit() {
        ...
        professor.addTab("educational request", new DemoList() {
            {
                columnsTitle = new String[] {"request", "student name", "Ok", "reject"};
                designTopics();
                for (Faculty.EducationalRequest request: Command.getInstance().getFaculty().getEducationalRequests()) {
                    addRow(request);
                }
            }

            private void addRow(Faculty.EducationalRequest request) {
                gbcFiller.gridy = rowsCounter++;
                columnCounter = 0;
